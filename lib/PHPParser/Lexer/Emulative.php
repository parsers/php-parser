...
class Emulative extends Lexer
{
    ...
    public function getNextToken(&$tokenvalue = null, &$v1 = null, &$v2 = null) {
        $tokenreturn = parent::getNextToken($tokenvalue, $v1, $v2);

        // replace new keywords by their respective tokens. This is not done
        // if we currently are in an object access (e.g. in $obj->namespace
        // "namespace" stays a T_STRING tokens and isn't converted to T_NAMESPACE)
        if (Parser::T_STRING === $tokenreturn && !$this->inObjectAccess) {
            if (isset($this->newKeywords[strtolower($tokenvalue)])) {
                $tokenreturn = $this->newKeywords[strtolower($tokenvalue)];
                return $tokenreturn;
            }
        // backslashes are replaced by T_NS_SEPARATOR tokens
        } elseif (92 === $tokenreturn) { // ord('\\')
            $tokenreturn = Parser::T_NS_SEPARATOR;
            return $tokenreturn;
        // keep track of whether we currently are in an object access (after ->)
        } elseif (Parser::T_OBJECT_OPERATOR === $tokenreturn) {
