...
class XML implements Serializer
{
    ...
    protected function _serialize($node) {
        if ($node instanceof \PHPParser\Node\NodeInterface) {
        	$type = $node->getType();
        	$type = str_replace('\\', '_', $type);
            $this->writer->startElement('node:' . $type);
            ...
            $this->writer->endElement();
        } elseif ($node instanceof \PHPParser\Comment\Comment) {
            $this->writer->startElement('comment');
            $this->writer->writeAttribute('isDocComment', $node instanceof \PHPParser\Comment\DocComment ? 'true' : 'false');
