...
use PHPParser\Node\RelativeNameNode;
...
class NameTest extends \PHPUnit_Framework_TestCase
{
    public function testConstruct() {
        $name = new NameNode(array('foo', 'bar'));
        $this->assertEquals(array('foo', 'bar'), $name->parts);

        $name = new NameNode('foo\bar');
        $this->assertEquals(array('foo', 'bar'), $name->parts);
    }

    public function testGet() {
        $name = new NameNode('foo');
    ...
    public function testIs() {
        ...
        $name = new FullyQualifiedNameNode('foo');
        $this->assertFalse($name->isUnqualified());
        $this->assertFalse($name->isQualified());
        $this->assertTrue ($name->isFullyQualified());
        $this->assertFalse($name->isRelative());

        $name = new RelativeNameNode('foo');
        $this->assertFalse($name->isUnqualified());
        $this->assertFalse($name->isQualified());
        $this->assertFalse($name->isFullyQualified());
